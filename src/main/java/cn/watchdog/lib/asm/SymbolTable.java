...
final class SymbolTable {
	...
	/**
	 * The total number of {@link cn.watchdog.lib.asm.SymbolTable.Entry} instances in {@link #entries}. This includes entries that are
	 * accessible (recursively) via {@link cn.watchdog.lib.asm.SymbolTable.Entry#next}.
	 */
	private int entryCount;
	/**
	 * A hash set of all the entries in this SymbolTable (this includes the constant pool entries, the
	 * bootstrap method entries and the type table entries). Each {@link cn.watchdog.lib.asm.SymbolTable.Entry} instance is stored at
	 * the array index given by its hash code modulo the array size. If several entries must be stored
	 * at the same array index, they are linked together via their {@link cn.watchdog.lib.asm.SymbolTable.Entry#next} field. The
	 ...
	 *
	 * @param hashCode a {@link cn.watchdog.lib.asm.SymbolTable.Entry#hashCode} value.
	 * @return the list of entries which can potentially have the given hash code. The list is stored
	 * via the {@link cn.watchdog.lib.asm.SymbolTable.Entry#next} field.
	 */
