...
public abstract class AbstractJavaScheduler implements SchedulerAdapter {
	...
	private final TronEconomy bootstrap;

	private final ScheduledThreadPoolExecutor scheduler;
	private final ForkJoinPool worker;

	public AbstractJavaScheduler(TronEconomy bootstrap) {
	...
	public void shutdownScheduler() {
		...
		try {
			if (!this.scheduler.awaitTermination(1, TimeUnit.MINUTES)) {
				this.bootstrap.getLogger().severe("Timed out waiting for the FloraCore scheduler to terminate");
	...
	private void reportRunningTasks(Predicate<Thread> predicate) {
		Thread.getAllStackTraces().forEach((thread, stack) -> {
			if (predicate.test(thread)) {
				this.bootstrap.getLogger()
						.warning("Thread " + thread.getName() + " is blocked, and may be the reason for the slow " +
								"shutdown!\n" +
	...
	private final class ExceptionHandler implements UncaughtExceptionHandler {
		...
		public void uncaughtException(Thread t, Throwable e) {
			AbstractJavaScheduler.this.bootstrap.getLogger()
					.warning("Thread " + t.getName() + " threw an uncaught exception");
			e.printStackTrace();
		}
	}
}
